{{-- resources/views/employer/pages/JobManagement/show.blade.php --}}
  ...
  <div class="card shadow-sm rounded-2 border-0">
      ...
      <div class="row g-4">
        ...
        <div class="col-md-6">
          ...
          <div class="mb-2 d-flex align-items-start">
            ...
            <div>
              ...
              {{ $job->salary_structure === 'project' ? 'Project' : ucfirst($job->salary_structure) }}
            </div>
          </div>
        ...
        <div class="col-md-6">
          ...
          <div class="d-flex align-items-start">
            <i class="bi bi-people-fill text-primary me-2 mt-1"></i>
            <div>
              <strong>Agents:</strong>
              <div class="d-flex flex-wrap gap-1 mt-1">
                @forelse($agents as $agent)
                  <span class="badge bg-light text-warning border">
                    <i class="bi bi-person-fill text-primary me-1"></i>{{ $agent->name }}
                  </span>
                @empty
                  <span class="text-white">No agents assigned</span>
                @endforelse
              </div>
            </div>
          </div>
      ...
      <div class="row g-4 mt-3">
        ...
        <div class="col-md-6">
          ...
          <div class="mb-2 d-flex align-items-start">
            ...
            <div><strong>Visa Sponsor:</strong> {{ $job->visa_sponsor ? 'Yes' : 'No' }}</div>
          </div>
        </div>
      </div>
    ...
    <div class="card-footer bg-light py-2 px-3 d-flex justify-content-between align-items-center">
      <small class="text-warning">
        ...
        Updated: {{ $job->updated_at->format('d M Y, h:i A') }}
      </small>
      <div>
        <a href="{{ route('employer.jobs.index') }}" class="btn btn-outline-secondary btn-sm me-1">
          <i class="bi bi-arrow-left me-1"></i> Back
        </a>
        <a href="{{ route('employer.jobs.edit', $job->id) }}" class="btn btn-primary btn-sm">
